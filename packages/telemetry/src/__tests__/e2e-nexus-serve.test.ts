...
describe("e2e: Nexus server integration", async () => {
  ...
  it("should create spans for traced HTTP calls to Nexus /health endpoint", async () => {
    ...
    const httpSpans = spans.filter((s) => s.name !== "templar.e2e.health_check");
    if (httpSpans.length > 0) {
      // HTTP spans should be children of our custom span
      for (const httpSpan of httpSpans) {
        expect(httpSpan.spanContext().traceId).toBe(healthSpan!.spanContext().traceId);
      }
    }
  });
  ...
  it("should maintain trace hierarchy through middleware wrapping Nexus calls", async () => {
    ...
    const beforeSpan = spans.find((s) => s.name === "templar.middleware.audit.before_turn");
    const afterSpan = spans.find((s) => s.name === "templar.middleware.audit.after_turn");
    ...
    console.log(`[e2e] Nexus integration: ${spans.length} spans created, traceId=${traceId}`);
  });
  ...
  it("should handle Nexus error responses without breaking trace", async () => {
    ...
    await withSpan("templar.e2e.error_test", { "test.type": "error-handling" }, async () => {
    ...
    console.log(`[e2e] Error test: response=${responseStatus}, spans=${spans.length}`);
  });
  ...
  it("should log OTel span context for audit correlation", async () => {
    ...
    await withSpan("templar.agent.turn", { "session.id": "e2e-session" }, async () => {
    ...
    console.log(`[e2e] Audit correlation: spanId=${capturedSpanId}, traceId=${capturedTraceId}`);
