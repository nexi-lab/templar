...
describe("Performance benchmarks", () => {
	...
	describe("memory usage", () => {
		...
		it("should handle middleware array creation efficiently", () => {
			...
			console.log(
				`  ✓ Created 100 agents with varying middleware in ${duration.toFixed(2)}ms`,
			);
		});
	});
	...
	describe("config spreading performance", () => {
		it("should spread config efficiently", () => {
			...
			console.log(
				`  ✓ Scaling factor: ${(largeTime / smallTime).toFixed(2)}x`,
			);
		});
	});
	...
	describe("regression prevention", () => {
		it("should maintain baseline performance", () => {
			...
			console.log(
				`  ✓ Throughput: ${Math.round(iterations / (duration / 1000))} agents/sec`,
			);
		});
	});
});
